...
export async function POST(request: Request) {
  try {
    ...
    // 🚨 [핵심 보완] 넘어온 데이터가 없거나, 배열이 아니거나, 빈 배열이면 AI를 호출하지 않고 곧바로 리턴!
    ...
    const diaryContext = diaries
      .filter((d: any) => d && d.content) // 데이터 유효성 검사 추가
      .map((d: any, idx: number) => `[기록 ${idx + 1}] 카테고리: ${d.category || '일상'}, 내용: ${d.content}`)
      .join('\n');

    // 🚨 [추가 보완] 글을 긁어모았는데 정작 알맹이(본문 텍스트)가 전부 비어있을 때 차단
...
4. 중요 키워드는 반드시 <strong style="color: #4f46e5;">강조단어</strong> 형태로 태그를 정확히 감싸서 강조해 주세요.
...
`;
    ...
    // VS Code 터미널에서 에러의 실체를 정확히 파악할 수 있도록 로깅
    console.error('메인 AI 분석 에러:', error);
    
    // 🚨 에러가 발생해도 500 에러를 뿜으며 프론트를 먹통으로 만들지 않고, 안전하게 문구를 돌려줌
    return NextResponse.json({ 
